...
[[nodiscard]] std::unique_ptr<shader_clippy::Rule> make_structured_buffer_stride_not_cache_aligned();
}  // namespace shader_clippy::rules
...
[[nodiscard]] std::vector<std::unique_ptr<Rule>> make_rule() {
    ...
    rules.push_back(shader_clippy::rules::make_structured_buffer_stride_not_cache_aligned());
    return rules;
}
