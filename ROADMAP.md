...
- **C++23 stdlib equivalents preferred over Microsoft GSL** (`std::span` instead of `gsl::span`; references / asserted bare pointers instead of `gsl::not_null`; `static_cast` with explicit assertions instead of `gsl::narrow`). GSL was discussed in early drafts but never linked into the build; revisit if a concrete need surfaces.
...
| 10.B — Slang sub-phase B (tree-sitter integration, v1.4+) | PLANNED | See [ADR 0021](docs/decisions/0021-slang-sub-phase-b-tree-sitter.md) |
