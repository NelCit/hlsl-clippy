    ...
    bool expect_success)
{
    ...
    if (raw_module == nullptr)
    {
        ...
        std::printf(
            "[ERR] %.*s: load failed (%zu diagnostics chars)\n%.*s\n",
    ...
    if (SLANG_FAILED(ep_res) || entry_point == nullptr)
    {
        ...
        std::printf(
            "[ERR] %.*s: entry-point lookup failed\n%.*s\n",
            static_cast<int>(entry_name.size()),
            entry_name.data(),
            static_cast<int>(diag.size()),
            diag.data());
    ...
    const SlangResult link_res =
        entry_point->link(linked.writeRef(), link_diag.writeRef());

    if (SLANG_FAILED(link_res) || linked == nullptr)
    {
        const std::string_view diag = diag_text(link_diag.get());
        std::printf(
            "[ERR] %.*s: %zu diagnostics\n%.*s\n",
            static_cast<int>(entry_name.size()),
            entry_name.data(),
            diag.size(),
            static_cast<int>(diag.size()),
    ...
    std::printf(
        "[OK]  %.*s compiled, %zu diagnostic char(s)\n",
        static_cast<int>(entry_name.size()),
        entry_name.data(),
        n_diag_chars);
...
int main()
{
    ...
    const bool good_ok = compile_shader(
        session.get(), "good_shader", "ps_main", k_good_source,
        /* expect_success = */ true);

    const bool bad_ok = compile_shader(
        session.get(), "bad_shader", "ps_bad", k_bad_source,
