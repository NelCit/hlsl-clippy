...
| `SHADER_CLIPPY_HOOK_FIX`                    | Set to `1` to auto-run `clang-format -i` and `git add` the staged files instead of rejecting the commit |

Example — rerun a rejected commit with auto-fix:

```sh
SHADER_CLIPPY_HOOK_FIX=1 git commit -s -m "feat: add foo rule"
```
