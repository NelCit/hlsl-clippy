...
canonical source is [`CHANGELOG.md`](https://github.com/NelCit/hlsl-clippy/blob/main/CHANGELOG.md)
in the repo root. Per-release artifacts live on the
[GitHub Releases](https://github.com/NelCit/hlsl-clippy/releases) page.
...
- **CLI rule engine + machine-applicable `--fix`** — rustc-style
  diagnostics; per-line / per-block / per-file `// hlsl-clippy: allow(...)`
  inline suppressions; idempotent quick-fix rewriting.
- **`.hlsl-clippy.toml` config** — per-rule severity, includes /
  excludes globs, per-directory `[[overrides]]`. Walk-up resolution
  bounded by the first `.git/` ancestor.
- **Reflection-aware rule packs** (Phase 3, [ADR 0012](https://github.com/NelCit/hlsl-clippy/blob/main/docs/decisions/0012-phase-3-reflection-infrastructure.md))
  — Slang reflection plumbed into `RuleContext` for buffer / sampler /
  root-signature / cbuffer-layout / SM 6.7-6.9 surface rules.
- **Control-flow + uniformity rules** (Phase 4, [ADR 0013](https://github.com/NelCit/hlsl-clippy/blob/main/docs/decisions/0013-phase-4-control-flow-infrastructure.md))
...
- **LSP server + VS Code extension** (Phase 5, [ADR 0014](https://github.com/NelCit/hlsl-clippy/blob/main/docs/decisions/0014-phase-5-lsp-architecture.md))
  — `hlsl-clippy-lsp` JSON-RPC server thinly wrapping `core`;
  TypeScript extension under the `nelcit` publisher; quick-fixes
  surfaced as VS Code code actions.
- **CI gate-mode polish** ([ADR 0015](https://github.com/NelCit/hlsl-clippy/blob/main/docs/decisions/0015-phase-6-launch-plan.md))
  ...
  of `$GITHUB_ACTIONS=true`. Drop the [example workflow](https://github.com/NelCit/hlsl-clippy/blob/main/docs/ci/lint-hlsl-example.yml)
...
- **Cross-platform release pipeline** — tag-driven builds on
  ...
  macOS notarization.
- **Hosted docs site** at <https://nelcit.github.io/hlsl-clippy/>.

### Known limitations (deferred)

- `cbuffer X : register(b0)` parses to an ERROR node in
  tree-sitter-hlsl; rules that need the binding fall back to Slang
  reflection. See [external/treesitter-version.md](https://github.com/NelCit/hlsl-clippy/blob/main/external/treesitter-version.md).
...
- VS Code Marketplace listing is gated on the `VSCE_PAT` repo
  ...
  via `code --install-extension hlsl-clippy-0.5.0.vsix` regardless.

## Quick links

- [Full changelog (main)](https://github.com/NelCit/hlsl-clippy/blob/main/CHANGELOG.md)
- [Releases page](https://github.com/NelCit/hlsl-clippy/releases)
