...
[[nodiscard]] bool literal_is_zero(std::string_view text) noexcept {
    ...
    while (i < text.size()) {
        if (!is_float_suffix_char(text[i])) return false;
        ++i;
    }
    return true;
}
...
[[nodiscard]] bool literal_is_one(std::string_view text) noexcept {
    ...
    ++i;
    if (i < text.size() && text[i] >= '2' && text[i] <= '9') return false;
    while (i < text.size() && text[i] == '0') ++i;
    if (i < text.size() && text[i] == '.') {
        ++i;
        while (i < text.size() && text[i] == '0') ++i;
    ...
    void on_tree(const AstTree& tree, RuleContext& ctx) override {
        ...
        engine.run(
            ...
            [&](const query::QueryMatch& match) {
                ...
                if (!literal_is_one(tree.text(then_n))) return;
                if (!literal_is_zero(tree.text(else_n))) return;

                // Condition must be a binary_expression with operator `>`.
                const char* cond_type = ::ts_node_type(cond);
                if (cond_type == nullptr ||
                    std::string_view{cond_type} != "binary_expression") return;
                if (binary_op(cond, tree.source_bytes()) != ">") return;

                // Extract x and threshold from the condition.
                const ::TSNode cond_left  =
                    ::ts_node_child_by_field_name(cond, "left", 4);
                const ::TSNode cond_right =
                    ::ts_node_child_by_field_name(cond, "right", 5);
                if (::ts_node_is_null(cond_left) || ::ts_node_is_null(cond_right)) return;
                ...
                diag.primary_span =
                    Span{.source = tree.source_id(), .bytes = ternary_range};
                diag.message =
                    std::string{"`"} + std::string{x_text} + " > " +
                    std::string{thr_text} + " ? 1.0 : 0.0` is the `step` idiom — "
                    "note: `step(" + std::string{thr_text} + ", " + std::string{x_text} +
                    ")` uses `>=` semantics; verify the boundary behaviour is correct";

                Fix fix;
                fix.machine_applicable = false;
                fix.description =
                    std::string{"replace with `step("} + std::string{thr_text} + ", " +
                    std::string{x_text} + ")` — confirm whether `>` or `>=` at the "
                    "threshold is intended (`step` uses `>=`)";
                TextEdit edit;
                edit.span        = Span{.source = tree.source_id(), .bytes = ternary_range};
                edit.replacement =
                    std::string{"step("} + std::string{thr_text} + ", " +
                    std::string{x_text} + ")";
