...
[[nodiscard]] bool literal_is_255(std::string_view text) noexcept {
    ...
    if (text[i] != '2' || text[i + 1] != '5' || text[i + 2] != '5') return false;
    i += 3;
...
void walk(::TSNode node, std::string_view bytes, const AstTree& tree, RuleContext& ctx) {
    ...
    if (node_kind(node) == "binary_expression") {
        ...
        if (op == "/" && node_kind(r) == "number_literal" &&
            literal_is_255(node_text(r, bytes))) {
            fires = true;
        ...
        if (fires) {
            ...
            fix.description =
                std::string{"if the source is a UNORM texture, drop the divide "
                            "(replace the expression with `"} +
                std::string{left_text} + "`); confirm the source format before "
                                          "applying";
            if (!left_text.empty()) {
                TextEdit edit;
                edit.span =
