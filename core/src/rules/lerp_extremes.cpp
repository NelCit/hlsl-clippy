    ...
    void on_tree(const AstTree& tree, RuleContext& ctx) override {
        ...
        engine.run(compiled.value(),
                   ...
                   [&](const query::QueryMatch& match) {
                       ...
                       if (::ts_node_is_null(fn) || ::ts_node_is_null(a) ||
                           ::ts_node_is_null(b)  || ::ts_node_is_null(t) ||
                           ::ts_node_is_null(call)) {
                           return;
                       }
                       if (tree.text(fn) != k_lerp_name) return;

                       const auto t_text = tree.text(t);
                       const bool is_zero = literal_is(t_text, 0.0);
                       const bool is_one  = literal_is(t_text, 1.0);
                       if (!is_zero && !is_one) return;
                       ...
                       fix.description = is_zero
                           ? std::string{"replace `lerp(a, b, 0)` with `a`"}
