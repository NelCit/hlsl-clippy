...
[[nodiscard]] std::size_t find_keyword(std::string_view text,
    ...
    while (pos <= text.size()) {
        ...
        if (ok_left && ok_right) return found;
        pos = found + 1;
    }
...
void walk(::TSNode node, std::string_view bytes, const AstTree& tree, RuleContext& ctx) {
    ...
    if (decl_like) {
        ...
        if (gs_pos != std::string_view::npos && vol_pos != std::string_view::npos) {
            ...
            const std::uint32_t vol_abs_lo =
                node_lo + static_cast<std::uint32_t>(vol_pos);
            ...
            if (end_strip < bytes.size() &&
                (bytes[end_strip] == ' ' || bytes[end_strip] == '\t')) {
                ++end_strip;
            }
