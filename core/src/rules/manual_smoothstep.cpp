...
[[nodiscard]] bool is_saturate_call(::TSNode node, std::string_view bytes) noexcept {
    ...
    if (node_text(fn, bytes) != "saturate")
        return false;
...
[[nodiscard]] std::optional<SmoothstepParams> try_match_saturate_decl(
    ::TSNode stmt, std::string_view bytes) noexcept {
    // Statement must be a declaration
    if (node_kind(stmt) != "declaration")
        return std::nullopt;
    ...
    // Declarator name
    const ::TSNode decl_name = ::ts_node_child_by_field_name(init_decl, "declarator", 10);
    if (node_kind(decl_name) != "identifier")
        return std::nullopt;
    const auto n_var = std::string{node_text(decl_name, bytes)};

    // Initializer value
    const ::TSNode init_value = ::ts_node_child_by_field_name(init_decl, "value", 5);
    if (!is_saturate_call(init_value, bytes))
        return std::nullopt;

    // The single argument to saturate should be `(t - a) / (b - a)`.
    const ::TSNode sat_args = ::ts_node_child_by_field_name(init_value, "arguments", 9);
    const ::TSNode sat_arg = ::ts_node_named_child(sat_args, 0);
    if (node_kind(sat_arg) != "binary_expression")
        return std::nullopt;
    if (binary_op(sat_arg, bytes) != "/")
        return std::nullopt;

    const ::TSNode num_node = ::ts_node_child_by_field_name(sat_arg, "left", 4);
    const ::TSNode den_node = ::ts_node_child_by_field_name(sat_arg, "right", 5);
    ...
    if (node_kind(num) != "binary_expression")
        return std::nullopt;
    if (binary_op(num, bytes) != "-")
        return std::nullopt;

    const auto t_text =
        std::string{node_text(::ts_node_child_by_field_name(num, "left", 4), bytes)};
    const auto a_text =
        std::string{node_text(::ts_node_child_by_field_name(num, "right", 5), bytes)};
    if (t_text.empty() || a_text.empty())
        return std::nullopt;
    ...
    if (node_kind(den) != "binary_expression")
        return std::nullopt;
    if (binary_op(den, bytes) != "-")
        return std::nullopt;

    const auto b_text =
        std::string{node_text(::ts_node_child_by_field_name(den, "left", 4), bytes)};
    const auto a2_text =
        std::string{node_text(::ts_node_child_by_field_name(den, "right", 5), bytes)};
    if (b_text.empty() || a2_text.empty())
        return std::nullopt;
...
[[nodiscard]] bool matches_hermite_form(const std::string& stripped,
    ...
    const bool has_pattern = stripped.find(two_n, after_paren) != std::string::npos ||
...
void walk_compound_statements(::TSNode node,
    ...
    if (node_kind(node) == "compound_statement") {
        ...
        for (std::size_t i = 0; i + 1 < stmts.size(); ++i) {
            ...
            if (!matches_hermite_form(stripped, params->n_var))
                continue;

            // Match! Fire on the pair of statements — span from start of decl
            // to end of second statement.
            const auto lo = static_cast<std::uint32_t>(::ts_node_start_byte(stmts[i]));
            const auto hi = static_cast<std::uint32_t>(::ts_node_end_byte(stmts[i + 1]));
            ...
            diag.message =
                std::string{
                    "hand-rolled cubic Hermite interpolation — "
                    "`"} +
                params->n_var + " = saturate((" + params->t_text + " - " + params->a_text +
                ") / (" + params->b_text + " - " + params->a_text + ")); " + params->n_var + " * " +
                params->n_var + " * (3 - 2 * " + params->n_var + ")` is `smoothstep(" +
                params->a_text + ", " + params->b_text + ", " + params->t_text + ")`";

            Fix fix;
            fix.machine_applicable = false;
            fix.description = std::string{"replace both statements with `smoothstep("} +
                              params->a_text + ", " + params->b_text + ", " + params->t_text +
                              ")` — check that `" + params->n_var +
...
public:
    ...
    void on_tree(const AstTree& tree, RuleContext& ctx) override {
        ...
        walk_compound_statements(::ts_tree_root_node(tree.raw_tree()), bytes, tree, ctx);
    }
};
